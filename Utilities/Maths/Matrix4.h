...
public:
	...
	static NCLMatrix4 BuildViewMatrix(const NCLVector3& from, const NCLVector3& lookingAt,
		const NCLVector3 up = NCLVector3(0, 1, 0));
	...
	inline friend std::ostream& operator<<(std::ostream& o, const NCLMatrix4& m)
	{
		...
		o << "\t\t" << m.values[12] << "," << m.values[13] << "," << m.values[14] << "," << m.values[15] << " )" << std
			::endl;
		return o;
	}
};
